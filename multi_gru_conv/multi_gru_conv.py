...
# next step: https://www.kaggle.com/harshaneigapula/cnn-lstm
...
if os.path.exists(EMBEDDING_MATRIX) and os.path.exists(TR_X) and os.path.exists(TR_Y) and os.path.exists(TS_X):
    ...
    # list_sentences_train = train["comment_text"].fillna("lilili").values
    ...
    # list_sentences_test = test["comment_text"].fillna("lilili").values
    ...
    # allEmbs = np.stack(embeddingDict.values())
    # word_index = tokenizer.word_index
    # numWords = min(TOP_WORDS, len(word_index))
    # embedding_matrix = np.random.normal(allEmbs.mean(), allEmbs.std(), (numWords, EMBEDDING_SIZE))
    # for word, i in word_index.items():
    #     if i >= TOP_WORDS: continue
    #     embedding_vector = embeddingDict.get(word)
    #     if embedding_vector is not None: embedding_matrix[i] = embedding_vector
...
# for model improvement: http://people.idsia.ch/~ciresan/data/cvpr2012.pdf
def get_model():
    inp = Input(shape = (MAX_LENGTH, ))
    x = Embedding(TOP_WORDS, EMBEDDING_SIZE, weights = [embedding_matrix], trainable = False)(inp)
    x = SpatialDropout1D(0.1)(x)

    rnnOut1 = Bidirectional(GRU(40, return_sequences = True, dropout = 0.1, recurrent_dropout = 0.1))(x)
    convOut1 = Conv1D(60, kernel_size = 3, strides = 1, activation = "relu")(rnnOut1)
    avgPool1 = GlobalAveragePooling1D()(convOut1)
    maxPool1 = GlobalMaxPooling1D()(convOut1) #MaxPooling1D()

    rnnOut2 = Bidirectional(GRU(80, return_sequences = True, dropout = 0.1, recurrent_dropout = 0.1))(x)
    convOut2 = Conv1D(120, kernel_size = 2, strides = 1, activation = "relu")(rnnOut2)
    avgPool2 = GlobalAveragePooling1D()(convOut2)
    maxPool2 = GlobalMaxPooling1D()(convOut2)

    x = concatenate([maxPool1, maxPool2, avgPool1, avgPool2])
...
#     earlyStop = EarlyStopping(monitor = 'val_loss', patience = PATIENCE, mode = 'min', verbose = 2)
#     return [checkpoint, RocAucVal, earlyStop]
def getCallbacks(valX, valY):
    RocAucVal = RocAucEvaluation(validation_data = (valX, valY), interval = 1)
...
if TEST:
    trainX, valX, trainY, valY = train_test_split(trX, trY, train_size = 1 - 1.0 / FOLDS)
    ...
    submission.to_csv("sub_multiGruConv%s_%s_topWords%s_maxLen%s_%s.csv" % (TR_TS, ("cleaned" if CLEAN else "uncleaned"), TOP_WORDS, MAX_LENGTH, W2V), index = False)
